...
shinyServer(function(input, output) {
  ...
  output$tocke <- renderPlot({
    ...
    tabela__1 <- df4 %>% filter(Drzava == input$drzava1)
  ...
  output$tocke2 <- renderPlot({
    ...
    tabela__2 <-  tabela4 %>% filter(Drzava == input$drzava2) 
    
    print(ggplot(data=tabela__2, aes(x = Leto, y = Tocke), group=1) + geom_point(col="cornflowerblue") + geom_line(col="cornflowerblue") +
            ylab("Število točk") + xlab("Leto")) + 
      theme_minimal()+
      theme(axis.title = element_text(size=13,face="bold"), axis.text = element_text(size=10), axis.text.x=element_text(angle=90, vjust=0.5, hjust=0.5))
  })
